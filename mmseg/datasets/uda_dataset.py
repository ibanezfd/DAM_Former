...
def get_rcs_class_probs(data_root, temperature, val, source=None):
    if val:
        name = 'flair_labels_train/sample_class_stats'+ source +'.json'
        #name = 'flair_labels_train/sample_class_stats.json'
    else:
        name = 'sample_class_stats.json'
    with open(osp.join(data_root, name), 'r') as of:
...
class UDADataset(object):
    ...
    def __init__(self, source, target, cfg):
        ...
        if self.rcs_enabled:
            ...
            if "Flair_Dataset" == cfg['source']['type']:
####            
                name = 'flair_labels_train/samples_with_class'+self.source.dataset_name+'.json'
                #name = 'flair_labels_train/samples_with_class.json'
                val = 1
            else:
                val = 0
                name = 'samples_with_class.json'

            self.rcs_classes, self.rcs_classprob = get_rcs_class_probs(
                cfg['source']['data_root'], self.rcs_class_temp, val, self.source.dataset_name)
