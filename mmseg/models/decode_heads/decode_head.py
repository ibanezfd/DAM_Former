...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        if sampler is not None:
            ...
            self.sampler = None
        if sampler_temp is not None:
            self.sampler_temp = build_pixel_sampler(sampler_temp, context=self)
        else:
            self.sampler_temp = None

        self.conv_seg = nn.Conv2d(channels, num_classes, kernel_size=1)
        self.conv_seg_t = nn.Conv2d(channels//2, num_classes, kernel_size=1)
        if dropout_ratio > 0:
    ...
    def cls_seg_temp(self, feat):
        """Classify each pixel for the temporal branch."""
        if self.dropout is not None:
            feat = self.dropout(feat)
        output = self.conv_seg_t(feat)
        return output
