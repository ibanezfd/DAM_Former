...
class DACS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        self.hrda= cfg['hrda']
        self.fdist_classes = None #cfg['imnet_feature_dist_classes']
        self.fdist_scale_min_ratio = cfg['imnet_feature_dist_scale_min_ratio']
        self.enable_fdist = False # self.fdist_lambda > 0 for only temp
        self.mix = cfg['mix']
        self.blur = cfg['blur']
    ...
    def calc_feat_dist(self, img, gt, feat=None):
        ...
        if isinstance(self.get_model(), HRDAEncoderDecoder) and \
            ...
            with torch.no_grad():
                ...
                feat_imnet = self.get_imnet_model().extract_feat(img, self.sp_image, self.dates)
    ...
    def get_pseudo_label_and_weight_complex(self, logits, logits_temp):
        #ema_softmax = torch.softmax(logits.detach(), dim=1)
        #ema_softmax_temp = torch.softmax(logits_temp.detach(), dim=1)
        #ema_softmax = ema_softmax + ema_softmax_temp*0.2 #exp1 0.7, 0.3
        #logits = (logits + 0.2*logits_temp) / 1.2
        ema_softmax = torch.softmax(logits.detach(), dim=1) #++logits_temp.detach()*0.2
        pseudo_prob, pseudo_label = torch.max(ema_softmax, dim=1)
        ps_large_p = pseudo_prob.ge(self.pseudo_threshold).long() == 1
        ps_size = np.size(np.array(pseudo_label.cpu()))
        pseudo_weight = torch.sum(ps_large_p).item() / ps_size
    ...
    def forward_train(self,
        ...
        #sp_image=None
        #dates=None
        self.sp_image=sp_image
        self.dates=dates
        #if type(sp_image) == list:
         #       raise TypeError(type(sp_image))
        log_vars = {}
        ...
        if sp_image == None:
            ...
            clean_losses = self.get_model().forward_train(
            img, img_metas, gt_semantic_seg, sp_image= sp_image, dates = dates, return_feat=True)
        ...
        if not self.source_only:
                # Generate pseudo-label for HR and temp
            ...
            ema_logits = self.get_ema_model().generate_pseudo_label(
                target_img, target_img_metas,sp_image, dates)
            seg_debug['Target'] = self.get_ema_model().debug_output
            
            
            if sp_image != None:
                #raise TypeError(ema_logits[0], ema_logits[1])
                pseudo_label, pseudo_weight = self.get_pseudo_label_and_weight_complex(
                    ema_logits[0], ema_logits[1])
                del ema_logits
            ...
            # Generate pseudo-label for non using sp_image
        ...
        if self.local_iter % self.debug_img_interval == 0 and \
            ...
            vis_img = img
            vis_img[:,:3,:,:] = torch.clamp(denorm(img[:,:3,:,:], means, stds), 0, 1)
        
            vis_trg_img = target_img
            vis_trg_img[:,:3,:,:] = torch.clamp(denorm(target_img[:,:3,:,:], means, stds), 0, 1)

            vis_mixed_img = mixed_img
            vis_mixed_img[:,:3,:,:] = torch.clamp(denorm(mixed_img[:,:3,:,:], means, stds), 0, 1)
            for j in range(batch_size):
                ...
                subplotimg(axs[0][0], vis_img[j][:3], 'Source Image')
                subplotimg(axs[1][0], vis_trg_img[j][:3], 'Target Image')
                subplotimg(
                    axs[0][1],
                    gt_semantic_seg[j],
                    'Source Seg GT',
                ...
                subplotimg(
                    ...
                    cmap='cityscapes')
                subplotimg(axs[0][2], vis_mixed_img[j][:3], 'Mixed Image')
                subplotimg(
                    ...
                    for k1, (n1, outs) in enumerate(seg_debug.items()):
                        ...
                        for k2, (n2, out) in enumerate(outs.items()):
                            if self.hrda:
                                subplotimg(
                                axs[k2][k1],
                                **prepare_debug_out(f'{n1} {n2}', outs['Img 1 Scale 1'][j], means, stds))
                            else:
                                subplotimg(
                                axs[k2][k1],
                                **prepare_debug_out(f'{n1} {n2}', outs["Image"][j], means, stds))
                    for ax in axs.flat:
                        ax.axis('off')
                    plt.savefig(
                        os.path.join(out_dir,
